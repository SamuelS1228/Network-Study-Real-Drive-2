...
def summary(
    ...
    # distance/time buckets adaptively
    metric_col = 'TimeMin' if 'TimeMin' in stores.columns else ('DistMiles' if 'DistMiles' in stores.columns else None)
    if metric_col:
        buckets = pd.cut(
            stores[metric_col],
            bins=[0, 100, 250, 500, 1000, float('inf')] if metric_col=='DistMiles' else [0, 60, 120, 240, 480, float('inf')],
            labels=['<100', '100‑250', '250‑500', '500‑1000', '>1000'] if metric_col=='DistMiles' else ['<1h', '1‑2h', '2‑4h', '4‑8h', '>8h'],
            right=False,
        )
        dist_summary = buckets.value_counts().sort_index()
        st.subheader('Store Distance/Time Distribution')
        st.bar_chart(dist_summary)
    else:
        st.info('No distance/time column found in assignment results')
